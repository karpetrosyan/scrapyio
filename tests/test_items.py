...
def test_build_items_middleware(monkeypatch):
    monkeypatch.setattr(
        CONFIGS,
        "ITEM_MIDDLEWARES",
        ["tests.test_items.TestMiddleWare", "tests.test_items.TestMiddleWare"],
    )
...
def test_orjson_dumping_with_option():
    expected_string = "\n".join(["{", '  "test": "test"', "}\n"])
    actual_string = orjson_dumps_wrapper(
        {"test": "test"}, option=orjson.OPT_APPEND_NEWLINE
    )
    assert actual_string == expected_string
...
async def test_item_manager_middlewares(monkeypatch):
    monkeypatch.setattr(
        CONFIGS, "ITEM_MIDDLEWARES", ["tests.test_items.TestItemMiddleWare"]
    )
    manager = ItemManager()
...
async def test_item_manager_middlewares_ignoring(monkeypatch):
    monkeypatch.setattr(
        CONFIGS, "ITEM_MIDDLEWARES", ["tests.test_items.TestIgnoreItemMiddleWare"]
    )
    manager = ItemManager()
    item = await manager._send_single_item_via_middlewares(item=TestItem(num=5))
    assert item is None
